...
def count_nice_strings(strings, is_nice_fn):
    return list(
        [is_nice_fn(string) for string in strings]
    ).count(True)


def is_nice_string(input):
    return is_nice_string_by_rules(
        input,
        (
            contains_three_vowels,
            has_double_letter,
            has_no_forbidden_strings
        ))


def contains_three_vowels(input):
    vowels = ("a", "e", "i", "o", "u")
    return sum([input.count(vowel) for vowel in vowels]) >= 3
...
def has_no_forbidden_strings(input):
    ...
    return all([input.count(forbidden_string) == 0 for forbidden_string in forbidden])


def is_nice_string_improved(input):
    return is_nice_string_by_rules(
        input,
        (
...
def has_pair_of_dupplicated_letters(input):
...
def has_repeating_letter_with_one_in_between(input):
    for index in range(len(input) - 2):
        three_letters = input[index: index + 3]
