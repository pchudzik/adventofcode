...
class Coordinate:
    ...
    def navigate(self, direction):
        directions = {
            "<": self.__navigate_west,
            ">": self.__navigate_east,
            "v": self.__navigate_south,
            "^": self.__navigate_north
        }
        return directions[direction]()

    def __navigate_west(self):
        return Coordinate(self.x - 1, self.y)

    def __navigate_east(self):
        return Coordinate(self.x + 1, self.y)

    def __navigate_south(self):
        return Coordinate(self.x, self.y - 1)

    def __navigate_north(self):
        return Coordinate(self.x, self.y + 1)
...
def find_number_of_houses(input):
    coordinates = [Coordinate(0, 0)]
    for direction in input:
        new_coordinate = coordinates[-1].navigate(direction)
        coordinates.append(new_coordinate)

    return len(set(coordinates))


def find_number_of_houses_with_robots(input, number_of_santas):
    coordinates = dict(map(
        lambda santa_index: (santa_index, [Coordinate(0, 0)]),
        range(number_of_santas)))
    for direction, santa_index in zip(input, list(range(number_of_santas)) * len(input)):
        new_coordinate = coordinates[santa_index][-1].navigate(direction)
        coordinates[santa_index].append(new_coordinate)

    return len(set(reduce(
        lambda result, collection: result + collection,
        coordinates.values(),
        [])))
...
if __name__ == "__main__":
    ...
    print("visited houses alone:", find_number_of_houses(input))
    print("visited houses with robot:",
          find_number_of_houses_with_robots(input, 2))
