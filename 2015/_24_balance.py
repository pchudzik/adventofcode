...
Of these, there are three arrangements that put the minimum (two) number of packages in the first group: 11 4, 10 5, and
8 7. Of these, 11 4 has the lowest quantum entanglement, and so it is selected.
...
def find_qes(packages, number_of_groups):
    group_size = sum(packages) / number_of_groups
    for x in range(1, len(packages)):
        qes = [
            quantum_entanglement(c)
            for c in combinations(packages, x)
            if sum(c) == group_size]
        if qes:
            return min(qes)
...
def quantum_entanglement(group1):
    return reduce(mul, group1)


if __name__ == "__main__":
    with open("_24_balance.txt") as file:
        packages = list(map(lambda x: int(x), file.readlines()))

        print("Part 1. Best quantum entanglement {}".format(find_qes(packages, 3)))
        print("Part 2. Best quantum entanglement {}".format(find_qes(packages, 4)))
