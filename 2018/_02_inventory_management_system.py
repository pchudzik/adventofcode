...
def count_letters(letters):
    ...
    return (
        1 if 2 in counts.values() else 0,
        1 if 3 in counts.values() else 0
    )


def checksum(ids):
    items = list(map(count_letters, ids))

    return sum(item[0] for item in items) * sum(item[1] for item in items)


def _compare_boxes(id1, id2):
    mixed_letter = None
    for i in range(len(id1)):
        if id1[i] != id2[i]:
            if mixed_letter is None:
                mixed_letter = i
            else:
                return False, None
...
def find_box(ids):
    for id1 in ids:
        for id2 in filter(lambda id: id != id1, ids):
            match, result = _compare_boxes(id1, id2)
            if match:
                return result
