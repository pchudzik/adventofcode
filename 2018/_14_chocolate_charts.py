...
Your puzzle answer was 2157138126.
...
Your puzzle answer was 20365081.

Both parts of this puzzle are complete! They provide two gold stars: **
"""


def _elf_position(current_position, score_board):
    return (current_position + 1 + int(score_board[current_position])) % len(score_board)


def _make_recipes(stop_condition):
    elf1 = 0
    elf2 = 1
    score_board = "37"

    while stop_condition(score_board):
        new_recipe = int(score_board[elf1]) + int(score_board[elf2])
        score_board += str(new_recipe)
        elf1 = _elf_position(elf1, score_board)
        elf2 = _elf_position(elf2, score_board)

    return score_board


def find_score_part1(recipe):
    recipe = int(recipe)
    score_board = _make_recipes(lambda score: len(score) < recipe + 10)
    return score_board[recipe:recipe + 10]


def find_score_part2(recipe):
    score_board = _make_recipes(lambda score: recipe not in score[-len(recipe)-1:])
    return score_board.index(recipe)


if __name__ == "__main__":
    puzzle = "894501"
    print(f"part 1: {find_score_part1(puzzle)}")
    print(f"part 2: {find_score_part2(puzzle)}")
