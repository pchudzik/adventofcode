...
Unfortunately, the computer you brought probably doesn't have enough memory to actually decompress the file; you'll have
to come up with another way to get its decompressed length.
...
def decompress(compressed, is_v2=False):
    tokens = _tokenize(compressed, is_v2=is_v2)
    return "".join(map(str, tokens))


def decompress_len(compressed, is_v2=False):
    tokens = _tokenize(compressed, is_v2=is_v2)
    return sum(map(len, tokens))


def _tokenize(compressed, is_v2=False):
    i = 0
    buffer = ""
    ...
    while i < len(compressed):
        if compressed[i] == "(":
            ...
            token_start = i + 1
            token_end = compressed.find(")", token_start)
            token = compressed[token_start:token_end]
            length, repeat = token.split("x")
            length, repeat = int(length), int(repeat)
            to_tokenize = compressed[token_end + 1:token_end + length + 1]
            if is_v2 and "(" in to_tokenize:
                yield from _tokenize(to_tokenize * repeat, is_v2=is_v2)
            else:
                yield to_tokenize * repeat

            i = token_end + len(to_tokenize) + 1
        else:
            buffer += compressed[i]
            i += 1
