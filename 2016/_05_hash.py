...
def find_password_door1(seed, *, password_len=8, start=0, step=1):
    pswd = ""
    index = start
    while len(pswd) < password_len:
        hash = _find_valid_hash(seed, index)
        index += step
        if hash:
            pswd += hash[5]
    return pswd
...
def find_password_door2(seed, *, password_len=8, start=0, step=1, stop_after=None):
    pswd = [None] * password_len
    index = start
    found_chars = 0
    while found_chars < password_len and (stop_after is None or found_chars < stop_after):
        hash = _find_valid_hash(seed, index)
        index += step
        if hash:
            position = int(hash[5], 16)
            if position < password_len and pswd[position] is None:
                found_chars += 1
                pswd[position] = hash[6]
    return pswd


def _find_valid_hash(seed, index):
    hash = hashlib.md5((seed + str(index)).encode("utf-8")).hexdigest()
    if hash[:5] == "00000":
        return hash
...
if __name__ == "__main__":
    ...
    print(f"p1 = {find_password_door1(puzzle_input)}")
    print(f"p2 = {''.join(find_password_door2(puzzle_input))}")
