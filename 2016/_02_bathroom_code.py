...
def find_code_standard_keypad(code):
    """
    1 2 3
    4 5 6
    7 8 9
...
def find_code_star_keypad(code):
    """
        1
      2 3 4
    5 6 7 8 9
...
def _find_code(code, keypad):
    last_digit = "5"
    result = ""
    for digit_path in code:
        last_digit = reduce(
            lambda last, step: keypad[last][step],
            digit_path.strip(),
            last_digit)
...
if __name__ == "__main__":
    with open("_02_bathroom_code.txt") as file:
        codes = file.readlines()
        print(f"p1 = {find_code_standard_keypad(codes)}")
        print(f"p2 = {find_code_star_keypad(codes)}")
