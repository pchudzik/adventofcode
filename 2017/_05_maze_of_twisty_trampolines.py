...
class Instruction:
    def __init__(self, jump, offset_modifier):
        self._jump = jump
        self.offset = 0
        self.offset_modifier = offset_modifier

    def jump(self):
        result = self._jump + self.offset
        self.offset += self.offset_modifier(result)
        return result


def part1_offset_counter(offset):
    return 1


def part2_offset_counter(jump_value):
...
class Program:
    ...
    @property
    def is_done(self):
        return self.index < 0 or self.index >= len(self.instructions)

    def step(self):
        instruction = self.instructions[self.index]
        self.index += instruction.jump()


def count_jumps_1(puzzle):
    return _count_jumps(puzzle, part1_offset_counter)


def count_jumps_2(puzzle):
    return _count_jumps(puzzle, part2_offset_counter)


def _count_jumps(puzzle, offset_modifier):
    program = Program([Instruction(offset, offset_modifier) for offset in puzzle])
    jumps = 0
...
if __name__ == "__main__":
    with open("_05_maze_of_twisty_trampolines.txt") as file:
