...
Each generator functions completely independently: they both go through values entirely on their own, only occasionally
handing an acceptable value to the judge, and otherwise working through the same sequence of values as before until they
find one.
...
This change makes the generators much slower, and the judge is getting impatient; it is now only willing to consider 5
million pairs. (Using the values from the example above, after five million pairs, the judge would eventually find a
...
def _generator(factor, starting_value, predicate=lambda v: True):
    previous_value = starting_value
    while True:
        previous_value = (previous_value * factor) % 2147483647
        if predicate(previous_value):
            yield previous_value


def picky_generator_a(starting_value):
    return _generator(
        generator_a_factor,
        starting_value,
        predicate=lambda v: v % 4 == 0)


def picky_generator_b(starting_value):
    return _generator(
        generator_b_factor,
        starting_value,
...
def generator_a(starting_value):
    return _generator(generator_a_factor, starting_value)


def generator_b(starting_value):
    return _generator(generator_b_factor, starting_value)


def is_lower_16_bits_equal(a, b):
    return bin(a)[-16:] == bin(b)[-16:]


def judge(number_of_pairs, generators):
    gen_a, gen_b = generators
    sum = 0
    for i in range(number_of_pairs):
        val_a = next(gen_a)
        val_b = next(gen_b)
        if is_lower_16_bits_equal(val_a, val_b):
            sum += 1
...
if __name__ == '__main__':
    gen_a_starting_value = 634
    gen_b_starting_value = 301

    gen_a = generator_a(gen_a_starting_value)
    gen_b = generator_b(gen_b_starting_value)

    picky_gen_a = picky_generator_a(gen_a_starting_value)
    picky_gen_b = picky_generator_b(gen_b_starting_value)

    print(f"part1: {judge(40_000_000, (gen_a, gen_b))}")
    print(f"part2: {judge(5_000_000, (picky_gen_a, picky_gen_b))}")
